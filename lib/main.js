...
window.onload = function() {
		...
		var requestTime = 25;
		if (window.location.hash.length > 1) {
				var hashTime = parseInt(window.location.hash.slice(1));
				if (hashTime && hashTime > 0) {
					requestTime = hashTime;
				}
		}
		myWorker.postMessage(['init', requestTime]);
		...
		var changeStateEffects = function(state) {
			...
			if (state.finish) {
				...
				// Log the time - save it into the local storage
				logTime(state.requestTime);
			}
		}
		...
		var logTime = function(time) {
			...
			if (typeof(Storage) !== 'undefined') {
				...
				// Try to get existing time and append to it 
				if (loggedTime = getLoggedTime()) {
					loggedTime.push(newEntry);
				} else {
					// Set a new time
					loggedTime =[newEntry];
				}
				...
				localStorage.setItem('pomodoroLog', JSON.stringify(loggedTime));
			}
		}

		var getLoggedTime = function() {
			// Check if localStorage is available
			if (typeof(Storage) !== 'undefined') {
				loggedTime = JSON.parse(localStorage.getItem("pomodoroLog"));
				return loggedTime;
			} else {
				return false;
			}
