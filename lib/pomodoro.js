...
Pomodoro.prototype = {
	...
	setState: function(states) {
		...
		// Add requestedTime, so its always available
		this.state['requestTime'] = this.pomodoroTime;
	...
	adjustTime: function(delta) {
		// Make sure the negative delta is not greater than the time left in the counter
		// e.g don't allow go to negative time
		if (delta < 0) {
			if (Math.abs(delta) > this.momentDuration.minutes()) {
				return true;
			}
		}
